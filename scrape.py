...
def check_health(report, previous_path="health.json"):
    ...
    for label, s in report["systems"].items():
        ...
        #
        # 但兩者不對稱，不能用同一條規則：
        #   次月永遠應該有整個月的日期，歸零一定是壞掉。
        #   本月會隨著月底自然歸零 —— 多數系統要提前幾天申請，月底一到
        #   就沒有可申請的當月日期了。太魯閣可當日申請所以還有資料，
        #   玉山／雪霸／林業署則會歸零，這是正常現象不是故障。
        #
        # 實際踩過：8/26 起這三個系統被誤判為壞掉，於是每輪都沿用舊資料，
        # 連帶把九月的名額也一起凍結了四天 —— 而九月才是真正會被預訂的日期。
        # 誤判的代價比漏判更大，因為它會讓好資料永遠更新不上去。
        if s["次月天數"] == 0 and (prev or {}).get("次月天數", 0) > 0:
            problems.append(
                f"{label}：次月天數歸零（上次 {prev['次月天數']} 天，節點還在但沒有日期資料）")
        if (s["本月天數"] == 0 and (prev or {}).get("本月天數", 0) > 0
                and s["次月天數"] == 0):
            problems.append(
                f"{label}：本月天數歸零（上次 {prev['本月天數']} 天，且次月同樣沒有資料）")
